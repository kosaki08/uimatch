...
uiMatch evaluates how closely an implemented UI matches a Figma design and returns concise, actionable hints to fix the gaps. It is invoked from a coding assistant (e.g., Claude Code) via a Skill that calls an in-process library (no external report files).
...
Coding Assistant → uiMatch Skill → uiMatch Core (TypeScript library)
├─ Figma API/MCP: fetch frame PNG (+ optional variables)
└─ Playwright (Chromium): capture target UI screenshot + computed styles
...
type DiffReport = {
  ...
  styleDiffs: Array<{
    ...
    properties: Record<string, { actual?: string; expected?: string; expectedToken?: string; delta?: number; unit?: string }>;
...
* All processing is local to the environment where the Skill runs.

## 11. Packaging

* **uimatch-core**: TypeScript library providing `compare(input): Promise<DiffReport>`
* **uimatch-skill**: Skill wrapper exposing `/uiMatch` commands and returning structured results to the calling session
