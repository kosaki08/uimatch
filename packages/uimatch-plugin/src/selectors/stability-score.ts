 ...
 */
function calculateLivenessScore(livenessResult?: LivenessResult): number {
  if (!livenessResult) {
    return 0.5; // Default score when liveness not checked
  }

  if (livenessResult.isAlive && livenessResult.state === 'visible') {
    return 1.0; // Element is alive and visible
  }

  if (livenessResult.state === 'hidden') {
    return 0.4; // Element exists but hidden
  }
...
export function calculateStabilityScore(
  ...
  if (params.livenessResult) {
    details.push(`  State: ${params.livenessResult.state}`);
    details.push(`  Alive: ${params.livenessResult.isAlive ? 'yes' : 'no'}`);
  }
